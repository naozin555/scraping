...
from service import ServiceWhatsNew, ServicePdfDl
...
class Controller(object):
    ...
    def execute(self):
        ...
        parsed_whats_new_html = ServiceWhatsNew.parse_whats_new(request)

        # 新着情報のある各審議会・委員会の資料のページのURLを取得する。
        base_url = "https://www.meti.go.jp"
        page_url_list, page_name_list = \
            ServiceWhatsNew.get_url(parsed_whats_new_html, base_url)
        dict_page = {}
        ...
        # 前回の新着情報ページとの差分を抽出する
        diff_page_url_list = \
            ServiceWhatsNew.comp_page_url_list(page_url_list, BASE_DIR)
        diff_page_name_list = []
        ...
        for i in range(len(diff_page_url_list)):
            ...
            base_url = url
            request = req.Request(url, headers=headers)
            parsed_html = ServicePdfDl.parse_page(request)

            # 各審議会・委員会の資料ページのリンク(aタグ)を取得する。
            link_list, text_list = \
                ServicePdfDl.get_url(parsed_html, base_url)

            # 各審議会・委員会の資料ページのPDFファイルのURLを取得する。
            pdf_url_list, pdf_filename_list = \
                ServicePdfDl.get_pdf(link_list, text_list)

            # PDFファイルのダウンロード
            dl_dir = BASE_DIR + diff_page_name_list[i] + '/'
            os.makedirs(dl_dir)
            ServicePdfDl.pdf_download(pdf_url_list, pdf_filename_list, headers, dl_dir)
